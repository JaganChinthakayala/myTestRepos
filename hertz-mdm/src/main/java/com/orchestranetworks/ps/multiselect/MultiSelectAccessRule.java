...
public class MultiSelectAccessRule implements AccessRule
{
	...
	public AccessPermission getPermission(Adaptation adaptation, Session session, SchemaNode node)
	{
		...
		if (!serviceID.equals(trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_SERVICE_ID)))
		{
			return AccessPermission.getReadWrite();
		}

		Repository repo = adaptation.getHome().getRepository();
		String parentRecordPK = trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_PARENT_RECORD_PK);
		if (parentRecordPK == null)
		{
			return AccessPermission.getReadWrite();
		}
		int joinTableParentPKPos = Integer.valueOf(
			trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_JOIN_TABLE_PK_POSITION))
			.intValue();
		AdaptationHome joinDataSpace = repo.lookupHome(HomeKey.forBranchName(trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_JOIN_TABLE_DATA_SPACE)));
		...
		Path joinTablePath = Path.parse(trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_JOIN_TABLE_PATH));
		Path joinTableFKPath = Path.parse(trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_JOIN_TABLE_FK_PATH));

		AdaptationTable joinTable = joinDataSet.getTable(joinTablePath);
		Path[] pkPaths = joinTable.getPrimaryKeySpec();
		RequestResult reqRes = joinTable.createRequestResult(Path.SELF.add(
			pkPaths[joinTableParentPKPos]).format()
			+ "='"
			+ parentRecordPK
			+ "' and "
		...
		if (reqRes.isEmpty())
		{
			...
			AdaptationHome parentTableDataSpace = repo.lookupHome(HomeKey.forBranchName(trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_PARENT_TABLE_DATA_SPACE)));
			Adaptation parentTableDataSet = parentTableDataSpace.findAdaptationOrNull(AdaptationName.forName(trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_PARENT_TABLE_DATA_SET)));
			AdaptationTable parentTable = parentTableDataSet.getTable(Path.parse(trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_PARENT_TABLE_PATH)));
			Adaptation parentRecord = parentTable.lookupAdaptationByPrimaryKey(PrimaryKey.parseString(parentRecordPK));
			return filter.accept(adaptation, parentRecord.createValueContext()) ? AccessPermission.getReadWrite()
