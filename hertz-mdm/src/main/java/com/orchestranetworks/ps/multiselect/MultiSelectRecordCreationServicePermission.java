...
public class MultiSelectRecordCreationServicePermission implements ServicePermission
{
	...
	public ActionPermission getPermission(SchemaNode node, Adaptation adaptation, Session session)
	{
		...
		TrackingInfoHelper trackingInfoHelper = MultiSelectUtil.createTrackingInfoHelper(createEmptyTrackingInfoHelper());
		trackingInfoHelper.initTrackingInfo(trackingInfo);
		String serviceID = trackingInfoHelper.getTrackingInfoSegment(MultiSelectUtil.SEGMENT_SERVICE_ID);
		return (serviceID != null && selectionServiceName.equals(serviceID)) ? ActionPermission.getEnabled()
