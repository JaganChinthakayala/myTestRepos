...
public final class MasterAccessRule implements AccessRule
{
		...
		SchemaNode aNode)
	{
		...
		final Adaptation apInstance = apHome.findAdaptationOrNull(AdaptationName.forName(dataSetName));
		if (apInstance == null)
		{
			return AccessPermission.getReadWrite();
		}
		...
		while ((ap = result.nextAdaptation()) != null)
		{
			...
			sessionPermissions = repository.createSessionPermissionsForUser(UserReference.forUser(user));
	...
	private String getPredicateForAccessPermission(Adaptation aAdaptation, AdaptationHome aHome)
	{
		...
		{
			predicate.append(" = '")
				.append(aAdaptation.getAdaptationName().getStringName())
				.append("')");
		}
		else
		{
