...
public abstract class UserTableLookupAccessRule implements AccessRule
{
	...
	protected boolean isLookupTableIgnoredForUser(Session session)
	{
		return WorkflowUtilities.isPermissionsUser(session)
			|| session.isUserInRole(Role.forSpecificRole(CommonConstants.ROLE_TECH_ADMIN));
	}
	...
	// Create the primary keys to look up the user with. It can return multiple primary key values if the record can result in multiple lookups.
	// For example, if looking up by country and the record's country field is multi-occurring, it will result in multiple primary keys to look for.
	...
	private Set<PrimaryKey> createLookupPrimaryKeys(
		...
		Adaptation adaptation)
	{
		HashSet<PrimaryKey> pks = new HashSet<PrimaryKey>();
		...
		if (recordValueNode.getMaxOccurs() > 1)
		{
			...
			{
				PrimaryKey pk = userLookupTable.computePrimaryKey(createLookupPrimaryKeyValues(
					user,
					recordValue,
					adaptation));
				if (pk != null)
