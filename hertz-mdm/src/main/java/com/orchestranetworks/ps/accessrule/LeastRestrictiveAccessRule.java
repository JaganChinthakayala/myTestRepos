...
/**
 * An AccessRule that will return the least restrictive permissions from the given access rules.
 * For example, if one rule returns Hidden and a 2nd rule returns ReadOnly, this will return ReadOnly.
 * The AccessRulesManager applies the most restrictive rule, so in this way you can essentially layer
		...
		for (int i = 0; i < rules.length && !AccessPermission.getReadWrite().equals(returnValue); i++)
		{
			...
			if (!AccessPermission.getHidden().equals(permission))
			{
				if (AccessPermission.getReadWrite().equals(permission)
					|| (AccessPermission.getReadOnly().equals(permission) && AccessPermission.getHidden()
						.equals(returnValue)))
				{
