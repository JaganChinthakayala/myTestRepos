...
import com.orchestranetworks.ps.enums.*;
...
public class ImportTrigger extends TableTrigger
{
		...
		throws OperationException
	{
		...
		final String parentDataSpace = adaptation.getString(BatchImportExportPaths._Root_BatchImport._DataSpace);
		final String dataSet = adaptation.getString(BatchImportExportPaths._Root_BatchImport._DataSet);
		final String tablePath = adaptation.getString(BatchImportExportPaths._Root_BatchImport._TablePath);
		final String filePath = adaptation.getString(BatchImportExportPaths._Root_BatchImport._FilePath);
		final String headerFormat = adaptation.getString(BatchImportExportPaths._Root_BatchImport._HeaderFormat);
		final String fieldSeparator = adaptation.getString(BatchImportExportPaths._Root_BatchImport._FieldSeparator);
		final String fileType = adaptation.getString(BatchImportExportPaths._Root_BatchImport._FileType);
		final String fileEncoding = adaptation.getString(BatchImportExportPaths._Root_BatchImport._FileEncoding);
		final String importMode = adaptation.getString(BatchImportExportPaths._Root_BatchImport._ImportMode);
		final int commitThreshold = adaptation.get_int(BatchImportExportPaths._Root_BatchImport._CommitThreshold);
		...
		homeCreationSpec.setKey(HomeKey.forBranchName(adaptation.getOccurrencePrimaryKey().format()
		...
		try
		{
			...
			{
				throw OperationException.createError("Path " + filePath + " is not a readable file");
			}
			...
			final ProgrammaticService mergeService = ProgrammaticService.createForSession(
				context.getSession(),
				parentAdaptationHome);
		...
		if (workingHome != null
			&& adaptation.getContainerTable()
				...
				.getNode(Path.parse("workingDataSpace")) != null)
		{
			pathValueMap.put(
