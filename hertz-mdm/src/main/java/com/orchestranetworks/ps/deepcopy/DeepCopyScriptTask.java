...
public abstract class DeepCopyScriptTask extends ScriptTaskBean
{
	...
	protected void initVariables(Repository repo) throws OperationException
	{
		...
		{
			throw OperationException.createError("fromDataSpace '" + fromDataSpace + "' not found.");
		}
		if (toDataSpace == null)
		{
			toDataSpaceRef = null;
		...
		else
		{
			...
			{
				throw OperationException.createError("toDataSpace '" + toDataSpace + "' not found.");
			}
		}
		Adaptation dataSetRef = fromDataSpaceRef.findAdaptationOrNull(AdaptationName.forName(dataSet));
		if (dataSetRef == null)
		{
