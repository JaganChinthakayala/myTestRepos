...
{
	private Map<Path, DeepCopyConfig> configMap = new HashMap<Path, DeepCopyConfig>();
	...
	{
		AdaptationTable table = adaptation.isSchemaInstance() ? adaptation.getTable(node.getPathInSchema())
	...
	{
		boolean permitted = config.isEnforcePermission() ? PermissionsUtil.isUserPermittedToCreateRecord(
			session,
			table) : true;
		if (!permitted)
		{
		...
		for (Map.Entry<Path, DeepCopyConfig> entry : fkConfigMap.entrySet())
		{
			...
			if (fkDataSpaceKey != null && !fkDataSpaceKey.equals(tableDataSpace.getKey()))
			{
				LoggingCategory.getKernel().error(
					"Deep copy is not allowed on a foreign key to another data space.");
				return false;
			}
		...
		for (Path selNodePath : selNodeConfigMap.keySet())
		{
			...
			if (!isUserPermitted(selNodeConfigMap.get(selNodePath), session, selNodeTable))
			{
				return false;
			}
		}
		Map<Path, DeepCopyConfig> associationConfigMap = config.getAssociationConfigMap();
		for (Path associationPath : associationConfigMap.keySet())
		{
			DeepCopyConfig associationConfig = associationConfigMap.get(associationPath);
			...
			if (linkTable == null)
			{
				...
				//				AdaptationHome assocDataSpace = (assocDataSpaceKey == null) ? tableDataSet.getHome()
