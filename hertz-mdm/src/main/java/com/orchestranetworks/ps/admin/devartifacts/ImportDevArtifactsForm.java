...
public class ImportDevArtifactsForm extends DevArtifactsForm
{
	...
	protected void writeExtraInputs(ServiceContext sContext) throws OperationException
	{
		writeCheckboxRow(
			...
			ImportDevArtifactsService.PARAM_REPLACE_MODE,
			"replaceModeSelected();");
		writeCheckboxRow(
			sContext,
			"Skip non-existing files",
			ImportDevArtifactsService.PARAM_SKIP_NONEXISTING_FILES,
			null);
	...
	protected void writeReplaceModeJS(ServiceContext sContext)
	{
		...
		writer.addJS_cr("  var chk = document.getElementById('"
			+ ImportDevArtifactsService.PARAM_REPLACE_MODE + "').checked;");
		...
		writer.addJS_cr("    if (allInputs[i].getAttribute('type') == 'checkbox' && (allInputs[i].getAttribute('id') == '"
			+ ImportDevArtifactsService.PARAM_SELECT_ALL_WORKFLOWS
			+ "' || (allInputs[i].getAttribute('id').length > "
			+ ImportDevArtifactsService.PARAM_WORKFLOW_PREFIX.length()
			+ " && allInputs[i].getAttribute('id').substring(0,"
			+ ImportDevArtifactsService.PARAM_WORKFLOW_PREFIX.length()
			+ ") == '"
			+ ImportDevArtifactsService.PARAM_WORKFLOW_PREFIX + "')))");
		writer.addJS_cr("    {");
	...
	protected void writeWorkflowSelectionPanel(ServiceContext sContext) throws OperationException
	{
		...
		{
			writeCheckboxRow(sContext, cbConfig.getLabel(), cbConfig.getComponentName(), null);
		}
		...
		throws OperationException
	{
		...
		List<String> workflowNames = ImportDevArtifactsConfigFactory.getWorkflowsFromFolder(workflowsFolder);
		...
		ArrayList<CheckboxConfig> cbConfigs = new ArrayList<CheckboxConfig>();
		// Loop through each name found in the folder
		for (String workflowName : workflowNames)
		{
			// Find the workflow model associated with that name
			Adaptation wfDataSet = wfDataSpace.findAdaptationOrNull(AdaptationName.forName(workflowName));
			String componentName = ImportDevArtifactsService.PARAM_WORKFLOW_PREFIX + workflowName;
			String label;
		...
		List<CheckboxConfig> workflowCBConfigs)
	{
		...
		{
			writer.addJS_cr("  document.getElementById('" + cbConfig.getComponentName()
				+ "').checked = document.getElementById('"
				+ ImportDevArtifactsService.PARAM_SELECT_ALL_WORKFLOWS + "').checked;");
	...
	protected void writeEnvironmentCopyPanel(ServiceContext sContext)
	{
		writeCheckboxRow(
			sContext,
			"Environment Copy",
			DevArtifactsService.PARAM_ENVIRONMENT_COPY,
			"copyEnvSelected();");
	...
	protected void writeEnvironmentCopyJS(ServiceContext sContext)
	{
		...
		writer.addJS_cr("  var chk = document.getElementById('"
			+ DevArtifactsService.PARAM_ENVIRONMENT_COPY + "').checked;");
		// Get all the input elements for the form
		writer.addJS_cr("  var allInputs = document.getElementsByTagName('input');");
	...
	protected void writeInformationPanel(ServiceContext sContext)
	{
		...
		writer.add_cr("  <p>In replace mode, all workflow models will be imported. Otherwise only the ones selected below will be imported (unless environment copy is selected).</p>");
		writer.add_cr("</div>");
	}
	...
	private File getWorkflowsFolder() throws IOException
	{
		...
		String workflowsFoldername = props.getProperty(DevArtifactsPropertyConstants.PROPERTY_WORKFLOWS_FOLDER);
