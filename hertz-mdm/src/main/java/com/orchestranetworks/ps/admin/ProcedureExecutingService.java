...
public class ProcedureExecutingService extends HttpServlet
{
		...
		throws ServletException, IOException
	{
		...
		{
			// Make sure only admins can execute. Servlets are exposed for anyone to call if they know the URL,
			// and we don't want any EBX user to execute it. If this isn't sufficient, we'll need to come up with a more elaborate
			// way to secure them.
			if (!(session.isUserInRole(Role.ADMINISTRATOR) || session.isUserInRole(CommonConstants.TECH_ADMIN)))
			{
