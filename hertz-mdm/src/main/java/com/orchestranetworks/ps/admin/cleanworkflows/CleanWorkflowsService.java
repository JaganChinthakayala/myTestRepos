...
/**
 * A service that terminates open workflows & closes child data spaces.
 */
...
public class CleanWorkflowsService extends HttpServlet
{
		...
		throws OperationException
	{
		...
		for (PublishedProcess workflowPublication : config.getWorkflowPublications())
		{
			...
			List<ProcessInstanceKey> processInstanceKeys = wfEngine.getProcessInstanceKeys(workflowPublication.getPublishedProcessKey());
	...
	protected void cleanDataSpaces(Repository repo, Session session, CleanWorkflowsConfig config)
		throws OperationException
	{
		AdaptationHome masterDataSpace = config.getMasterDataSpace();
		@SuppressWarnings("unchecked")
