...
public abstract class DevArtifactsForm
{
	...
	protected void writeHiddenInputs(ServiceContext sContext)
	{
		...
		{
			writer.add_cr("<input type='hidden' id='" + DevArtifactsService.PARAM_PROPERTIES_FILE
				+ "' name='" + DevArtifactsService.PARAM_PROPERTIES_FILE + "' value='"
				+ propertiesFile + "'/>");
		}
	}
	 ...
	 */
	protected void writeCheckboxRow(ServiceContext sContext, String label, String id, String onclick)
	{
		UIServiceComponentWriter writer = sContext.getUIComponentWriter();
		writer.startFormRow(new UIFormLabelSpec(label));
		writer.add_cr("<input type='checkbox' id='" + id + "' name='" + id + "' value='true'"
