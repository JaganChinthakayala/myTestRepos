...
public abstract class CopyComputedValuesProcedure implements Procedure
{
	...
	// These aren't used by the procedure, but can be used by the service / script task executing the procedure
	...
	protected List<Path> triggerEnabledTablePaths = new ArrayList<Path>();;
	...
	{
		Adaptation dataSet = pContext.getAdaptationHome().findAdaptationOrNull(
			AdaptationName.forName(dataSetName));
