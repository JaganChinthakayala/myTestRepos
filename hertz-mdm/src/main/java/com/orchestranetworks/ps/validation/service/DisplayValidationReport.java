...
public class DisplayValidationReport extends GenerateValidationReport
{
	...
	private static final String OCCURRENCE_STYLE = "padding:5px;" + "height:20px;"
		+ "border:1px solid lightgrey;" + "text-align:center;" + "vertical-align:center;"
	...
	public DisplayValidationReport(
		...
		Severity minSeverity) throws ServletException
	{
		super(
			request,
			response,
		...
		throws IOException
	{
		...
		uiComponentWriter.add_cr("        var oldPageAnchor = document.getElementById('pg'+this.currentPage);");
		uiComponentWriter.add_cr("        oldPageAnchor.className = 'pg-normal';");

		uiComponentWriter.add_cr("        this.currentPage = pageNumber;");
		uiComponentWriter.add_cr("        var newPageAnchor = document.getElementById('pg'+this.currentPage);");
		...
		uiComponentWriter.add_cr("    var pagerHtml = '<span onclick=\"' + pagerName + '.prev();\" class=\"pg-normal\"> &#171 Prev </span> | ';");
		uiComponentWriter.add_cr("        for (var page = 1; page <= this.pages; page++)");
		uiComponentWriter.add_cr("            pagerHtml += '<span id=\"pg' + page + '\" class=\"pg-normal\" onclick=\"' + pagerName + '.showPage(' + page + ');\">' + page + '</span> | ';");
		uiComponentWriter.add_cr("        pagerHtml += '<span onclick=\"'+pagerName+'.next();\" class=\"pg-normal\"> Next &#187;</span>'; ");
		...
		uiComponentWriter.add_cr("<table id=\"results\" class=\"text\" style=\"" + TABLE_STYLE
			+ "\">");
		...
		{
			uiComponentWriter.add_cr("<tr style=\"padding:5px;height:20px;border:1px solid lightgrey;text-align:center;vertical-align:center;white-space:nowrap;cursor:pointer;\">");

			final ValidationErrorElement validationErrorElement = list.get(i);

			final UIHttpManagerComponent managerComponent = sContext.createWebComponentForSubSession();
			...
			final UIButtonSpecJSAction buttonCreateOrganisation = uiComponentWriter.buildButtonPreview(
				url,
			...
			if (validationErrorElement.getRecord().isTableOccurrence())
			{
				...
				uiComponentWriter.add_cr("</td>");
				uiComponentWriter.add_cr("<td style=\"" + OCCURRENCE_STYLE + "\">"
					+ validationErrorElement.getMessage() + "</td>");
				uiComponentWriter.add_cr("<td style=\"" + OCCURRENCE_STYLE + "\">"
			...
			{
				uiComponentWriter.add_cr("<td style=\"" + OCCURRENCE_STYLE + "\">"
					+ validationErrorElement.getRecord().getLabelOrName(sContext.getLocale()));
				uiComponentWriter.add_cr("</td>");
		...
		super.generateUI(sContext, list);
	}
}
