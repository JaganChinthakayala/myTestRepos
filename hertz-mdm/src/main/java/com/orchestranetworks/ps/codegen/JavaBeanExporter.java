...
public class JavaBeanExporter extends FileExporter
{
	...
	public JavaBeanExporter(
		...
		SchemaNode tableNode) throws IOException
	{
		super(
			parentDir,
			exportDir,
	...
	private void writeClass(BufferedWriter writer) throws IOException
	{
		...
		writer.newLine();
		writeIndent(writer, 2).append("public ")
			.append(tableName)
			.append(" evaluate(Adaptation record) {");
		writer.newLine();
	...
	private void writeAccessMethods(BufferedWriter writer, Field field) throws IOException
	{
		...
		if (field.relationship)
		{
			...
			{
				writeIndent(writer, 2).append(
					"List<Adaptation> list = AdaptationUtil.evaluateSingle(");
				writer.newLine();
		...
		if (!field.relationship || !field.collection)
		{
			...
			writer.newLine();
			writeIndent(writer, 2).append(
				"ValueContextForUpdate updateContext = getValueContextForUpdate();");
			writer.newLine();
	...
	private void writeFieldGetter(BufferedWriter writer, Field field, String path, boolean useRecord)
		throws IOException
	{
	...
	private void writeFieldSetter(BufferedWriter writer, Field field) throws IOException
	{
		...
		if (field.relationship)
		{
			if (field.fk && !field.collection)
			{
				...
				writeIndent(writer, 3).append("PrimaryKey pk = ")
					.append(field.name)
