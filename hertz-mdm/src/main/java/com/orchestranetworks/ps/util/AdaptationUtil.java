...
{
	/**
	 * Given a record <code>Adaptation</code> and a <code>Locale</code>, this function will produce a string representation
	 * of the record.
	 */
	 ...
	 */
	public static final UnaryFunction<Adaptation, String> GetDefaultLabel = GetLabel.bindRight(Locale.getDefault());

	/**
	 * Given a <code>SchemaNode</code> and a <code>Locale</code>, this function will produce a string representation
	 * of the record.
	 */
	...
	public static final UnaryFunction<SchemaNode, String> GetNodeDefaultLabel = GetNodeLabel.bindRight(Locale.getDefault());
	...
	/**
	 * Given a record and function that will return a parent record for a given record, and an optional
	 * function to get the display-name for a given record (label is used by default), traverse the parent
	 * path implied by the getParent function and return a string representation of that path if it creates
	 * a cycle, null otherwise.
	 * @param record
	 * @param recordContext (for the record being modified, used to evaluate the first parent only)
	 * @param getParent - binary function to get parent from record or context (pref to context)
	 * @param toString
		...
		UnaryFunction<Adaptation, String> toString)
	{
		...
		Set<Adaptation> visited = new LinkedHashSet<Adaptation>();
		...
		CompareOper[] compareOpers)
	{
		...
		for (int i = 0; i < paths.length; i++)
		{
			if (i > 0)
				sb.append(" and ");
			sb.append("(");
			appendPredicate(sb, tableNode, paths[i], values[i], compareOpers == null ? null
				: compareOpers[i]);
			sb.append(")");
		}
		return sb.toString();
		...
		CompareOper compareOper)
	{
		...
		buildPredicate(sb, path, valueString(node, value), compareOper);
	}

	private static String valueString(SchemaNode node, Object value)
	{
		if (value == null)
			return null;
		String valString = StringUtils.replace(node.formatToXsString(value), "'", "''");
	...
	/**
	 * Given a table and a path to the 'name' field, determine whether the given name exists in the table 
	 */
		...
		Path sortField)
	{
		...
		if (sortField != null)
			reqRes = table.createRequestResult(predicate, new RequestSortCriteria().add(sortField));
		else
		...
		CompareOper[] compareOpers)
	{
		...
		RequestResult rr = table.createRequestResult(createQueryPredicate(
			table.getTableOccurrenceRootNode(),
			paths,
			values,
			compareOpers));
		try
