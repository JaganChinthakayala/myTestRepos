...
{
	public static <T> void addAll(Collection<T> toCollection, Collection<? extends T> fromCollection)
	{
	...
	/**
	 * Returns a singleton list of the specified object, except for null it will return an empty list.
	 */
	...
	public static <T> T getLastOrNull(Collection<T> collection)
	{
		...
		return size > 0 ? new ArrayList<T>(collection).get(size - 1) : null;
	}
	...
	/**
	 * If the collection is empty null is returned. If there is only 1 item in the collection it is returned. If there
	...
	/**
	 * If the iterator is empty null is returned. If there is only 1 item in the iterator it is returned. If there is
	...
	{
		List<T> reverse = new ArrayList<T>(values);
		...
		final int endAt)
	{
		...
		{
			throw new IndexOutOfBoundsException("Start Index: " + startAt + ", End Index: " + endAt);
		}
	...
	/**
	 * Creates a new array of type clazz. If clazz is an array type, an array of the type of the clazz array is created,
	 * not an array of arrays. clazz.getComponentType() is used to extract the type if clazz is an array.
	 */
	@SuppressWarnings("rawtypes")
	...
	/**
	 * Calls createArray(clazz, size) and then copies the values from defaultValues into the new array.
	 */
	@SuppressWarnings("rawtypes")
	...
	/**
	 * Creates an immutable map with 1 entry. The key will be argName and the value will be argValue.
	 * @return The 1 element immutable map.
	 */
	public static Map<String, Object> createKeyValMap(String argName, Object argValue)
	{
		return new ArrayMap<String, Object>(new Object[] { argName, argValue });
	}
	...
	{
		return concurrent ? new ConcurrentHashMap<K, V>() : new HashMap<K, V>();
	}
	...
	{
		return concurrent ? new ConcurrentHashMap<K, V>(initialCapacity) : new HashMap<K, V>(
			initialCapacity);
	}
	...
	/**
	 * Flattens a collection of collections (of collections ...) into a single collection. Returns the flattened
	 * collection.
	 */
	...
	{
		Collection<T> flattened = new ArrayList<T>(collection.size());
	 ...
	 * @param toFill The collection to fill with the flattened elements.
	 * @param root The root collection to traverse and flatten.
	 * @param skipNulls If true, embedded nulls are skipped, if false they are included in the resulting collection
	 */
	...
	/**
	 * Concatenates 2 lists. If list 1 is empty list 2 is returned, and vice versa. If neither list is empty a new list
	...
	public static <T> List<? extends T> concat(List<? extends T> c1, List<? extends T> c2)
	{
		...
		List<T> newlist = new ArrayList<T>(c1.size() + c2.size());
		newlist.addAll(c1);
		newlist.addAll(c2);
		return newlist;
	}
		...
		Collection<? extends T> subCollection)
	{
		...
		for (T subItem : subCollection)
		{
			...
			if (!ObjectUtils.equals(fullItem, subItem))
			{
				return false;
			}
		}
	...
	/**
	 * Compares elements from a collection and sorts them into the order they appear in the list passed into the
	 * constructor.
	 */
	@SuppressWarnings("rawtypes")
	public static class CompareByPosition implements Comparator
	{
		private final UnaryFunction keyFunction;
		private List inCollection = null;
		private boolean ascending = true;

		public CompareByPosition(List inCollection, boolean ascending)
		{
			this(UnaryFunction.identity(), inCollection, ascending);
		}

		public CompareByPosition(UnaryFunction keyFunction, List inCollection, boolean ascending)
		{
		...
		{
			int i1 = inCollection.indexOf(keyFunction.evaluate(o1));
			int i2 = inCollection.indexOf(keyFunction.evaluate(o2));
			if (i1 == i2)
			{
				return 0;
			}
	...
	public static int indexInArray(Object[] array, Object object)
	{
		...
		{
			if (ObjectUtils.equals(array[i], object))
			{
				return i;
			}
		}
	...
	/**
	 * Remove a value from a map -- this is useful if the key derived from the object has changed (out from under you).
	 * Note, if the object is mapped more than once in the map, all instances of it will be removed.
	 * @param map from which to find value
	 * @param value an object in the values of the map
	...
	/**
	 * Remove a value from a map and collect the keys that use the value -- this is useful if the key derived from the
	 * object has changed (out from under you). Note, if the object is mapped more than once in the map, all instances
	 * of it will be removed.
	 * @param map from which to find value
	 * @param value an object in the values of the map
	 * @param keys a collection to fill with the keys that map to the value
	 * @return true if the map has been modified
	 */
		...
		Collection<? extends T> coll2)
	{
		...
		List<T> result = new ArrayList<T>(coll1);
		List<T> subtractor = new ArrayList<T>(coll1);
		for (T element : coll2)
		{
	...
	/**
	 * Given a collection, a function to turn elements in the collection into strings, and a separator,
