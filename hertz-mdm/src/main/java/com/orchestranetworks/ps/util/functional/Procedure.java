...
/**
 * A functor that takes no arguments and returns no value. <p/> Note that this functor implements the Runnable
 * interface, making all <code>Procedure</code>s immediately usable in a number of contexts (Swing, Jelly, etc.). </p>
 * <p/> credit: org.apache.commons.functor. copied to extend functionality for ON.
 */
