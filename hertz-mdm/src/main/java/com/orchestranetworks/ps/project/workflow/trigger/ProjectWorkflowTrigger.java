...
/**
 * A trigger for project-related workflows. It stores information about the users & roles associated
...
public class ProjectWorkflowTrigger extends WorkflowTriggerBean implements ProjectPathCapable
{
	...
	public void handleAfterProcessInstanceStart(
		WorkflowTriggerAfterProcessInstanceStartContext context) throws OperationException
	{
		final ProcessInstance processInstance = context.getProcessInstance();
		Repository repo = context.getRepository();
		Session session = context.getSession();
		// If this is a subworkflow, refresh cache for its parent workflow so that it will contain the keys for its subworkflows
		if (processInstance.isSubWorkflow())
		{
			WorkflowEngine wfEngine = WorkflowEngine.getFromRepository(repo, session);
			ProcessInstance parentWorkflow = wfEngine.getProcessInstance(processInstance.getParentKey());
		...
		if (isTrackingWorkflowEvents())
		{
			...
			ProcedureExecutor.executeProcedure(
				proc,
				session,
				lookupWorkflowEventDataSpace(dataContext, repo));
		}
	}
		...
		throws OperationException
	{
		...
		{
			users = new HashSet<UserReference>();
		...
		if (isTrackingWorkflowEvents())
		{
			...
			Procedure proc = new Procedure()
			{
				...
				public void execute(ProcedureContext pContext) throws Exception
				{
					// beforeStart can get called before afterCreation, so it's possible that the record already was created
					// This is handled by createWorkflowEvent also, but best to capture it here since it's an expected situation
	...
	private Adaptation createWorkflowEvent(
		...
		WorkItem workItem,
		Integer stepId,
		Date date) throws OperationException
	{
		ProjectPathConfig pathConfig = getProjectPathConfig();
		...
		if (workflowEventRecord == null)
		{
			...
			if (workItem != null)
			{
				...
				LOG.warn("No event record found for workflow "
		...
		{
			LOG.warn("Event record already found for workflow "
		...
		AdaptationTable workflowEventTable = dataSet.getTable(pathConfig.getProjectWorkflowEventTablePath());
		...
		updateWorkflowEventsWithProject(
			...
			(String) pathValueMap.get(pathConfig.getProjectWorkflowEventMasterWorkflowInstanceKeyFieldPath()));
	...
	private void endWorkflowEvent(
		...
		WorkItem workItem,
		Integer stepId,
		Boolean isAccepted) throws OperationException
	{
		Date now = new Date();
		ProjectPathConfig pathConfig = getProjectPathConfig();
		Repository repo = pContext.getAdaptationHome().getRepository();
		...
		else
		{
			...
			{
				Locale locale = session.getLocale();
				LOG.warn("No event record found for work item "
			...
			pathValueMap.put(
				pathConfig.getProjectWorkflowEventWorkItemIsAcceptedFieldPath(),
				isAccepted);
			...
			// (Work item records only exist during scope of the work item and pk is reused next time through.)
			pathValueMap.put(pathConfig.getProjectWorkflowEventWorkItemInstanceKeyFieldPath(), null);
		}

		pathValueMap.put(pathConfig.getProjectWorkflowEventCompleteDateTimeFieldPath(), now);
		ModifyValuesProcedure.execute(pContext, record, pathValueMap, true, false);
	}
		...
		Repository repo) throws OperationException
	{
		...
		String processInstancePK = convertProcessInstanceKeyToPK(processInstance.getProcessInstanceKey());
		...
		Repository repo) throws OperationException
	{
		...
		String predicate = pathConfig.getProjectWorkflowEventWorkItemInstanceKeyFieldPath()
			.format() + "='" + workItemPK + "'";
		return workflowEventTable.lookupFirstRecordMatchingPredicate(predicate);
	}
	...
	private AdaptationHome lookupWorkflowEventDataSpace(
		...
		Repository repo) throws OperationException
	{
		return WorkflowUtilities.getDataSpace(
			dataContext,
			repo,
			WorkflowConstants.PARAM_WORKING_DATA_SPACE);
	}
	...
	private Adaptation lookupWorkflowEventDataSet(DataContextReadOnly dataContext, Repository repo)
		throws OperationException
	{
		return WorkflowUtilities.getDataSet(
			dataContext,
			repo,
			WorkflowConstants.PARAM_WORKING_DATA_SPACE);
	}
		...
		UserReference user) throws OperationException
	{
		...
		if (record == null)
		{
			...
			createWorkflowEvent(pContext, dataContext, processInstance, workItem, stepId, null);
		}
		HashMap<Path, Object> pathValueMap = new HashMap<Path, Object>();
		pathValueMap.put(
		...
		Session session) throws OperationException
	{
		...
		pathValueMap.put(
			pathConfig.getProjectWorkflowEventWorkItemInstanceKeyFieldPath(),
			workItemKeyStr);
		...
		Locale locale)
	{
		...
		Adaptation wfDataSet = wfDataSpace.findAdaptationOrNull(AdaptationName.forName(workflowModelName));
		return wfDataSet == null ? null : wfDataSet.getLabel(locale);
	}

	@Override
	public void handleBeforeProcessInstanceTermination(
		WorkflowTriggerBeforeProcessInstanceTerminationContext context) throws OperationException
	{
		final ProcessInstance processInstance = context.getProcessInstance();
		...
		Repository repo = context.getRepository();
		Session session = context.getSession();
		// If this is a subworkflow, refresh cache for its parent workflow so that it will no longer contain the key for this subworkflow
		if (processInstance.isSubWorkflow())
		{
			WorkflowEngine wfEngine = WorkflowEngine.getFromRepository(repo, session);
			ProcessInstance parentWorkflow = wfEngine.getProcessInstance(processInstance.getParentKey());
		...
		if (isTrackingWorkflowEvents())
		{
			...
			// We don't want to error out, for backwards compatibility with existing workflows that don't have that step
			...
			{
				String workingDataSpaceName = dataContext.getVariableString(WorkflowConstants.PARAM_WORKING_DATA_SPACE);
				if (workingDataSpaceName == null)
				{
					LOG.warn("Working data space of workflow "
				...
				{
					LOG.warn("Working data space "
						+ workingDataSpaceName
						+ " of workflow "
						+ processInstance.getLabel()
							.formatMessage(context.getSession().getLocale())
				...
				Procedure proc = new Procedure()
				{
					...
					public void execute(ProcedureContext pContext) throws Exception
					{
						Adaptation projectRecord = ProjectWorkflowUtilities.getProjectRecord(
							...
							WorkflowConstants.PARAM_WORKING_DATA_SPACE,
							false);
						// If there's no project record then it must have been canceled before creating the project or
						// before merging it to master.
						// If it's the master workflow, delete it, which will delete its child records.
						// If it's a subworkflow, ignore because when the master ends it will clean it up.
						if (projectRecord == null)
						{
	...
	private void deleteMasterWorkflowEvent(
		ProcedureContext pContext,
		DataContextReadOnly dataContext,
		ProcessInstance processInstance) throws OperationException
	{
		...
		{
			DeleteRecordProcedure.execute(pContext, workflowEventRecord, true, false, true);
		}
	}

	@Override
	public void handleBeforeWorkItemAllocation(
		WorkflowTriggerBeforeWorkItemAllocationContext context) throws OperationException
	{
	...
	public void handleBeforeWorkItemDeallocation(
		WorkflowTriggerBeforeWorkItemDeallocationContext context) throws OperationException
	{
		Session session = context.getSession();
		Repository repo = context.getRepository();
		final ProcessInstance processInstance = context.getProcessInstance();
	...
	public void handleBeforeWorkItemReallocation(
		WorkflowTriggerBeforeWorkItemReallocationContext context) throws OperationException
	{
		handleAllocationOrReallocation(context);
	}
	...
	public void handleBeforeWorkItemTermination(
		WorkflowTriggerBeforeWorkItemTerminationContext context) throws OperationException
	{
		if (isTrackingWorkflowEvents())
		{
			final DataContextReadOnly dataContext = context;
	...
	private void updateWorkflowEventsWithProject(
		...
		String projectPK,
		String masterWorkflowInstancePK) throws OperationException
	{
		ProjectPathConfig pathConfig = getProjectPathConfig();
		if (projectPK != null)
		{
			Path projectFieldPath = pathConfig.getProjectWorkflowEventProjectFieldPath();
			RequestResult reqRes = workflowEventTable.createRequestResult("osd:is-null("
				+ projectFieldPath.format() + ") and "
				+ pathConfig.getProjectWorkflowEventMasterWorkflowInstanceKeyFieldPath().format()
