...
import com.orchestranetworks.workflow.MailSpec.NotificationType;
...
public class ProjectWorkflowUtilities extends WorkflowUtilities
{
	public static void notifyProjectTeam(
		...
		ProjectPathConfig pathConfig) throws OperationException
	{
	...
	{
		return getProjectRecord(dataContext, repo, pathConfig, dataSpaceParam, true);
	}
	...
	public static Adaptation getProjectRecordFromSessionInteraction(
		...
		ProjectPathConfig pathConfig) throws OperationException
	{
		Adaptation projectRecord = WorkflowUtilities.getRecordFromSessionInteraction(
			interaction,
			repo);
		if (!isProjectRecord(projectRecord, pathConfig))
		{
	...
	{
		List<Adaptation> subjectRecords = getSubjectRecords(dataContext, repo, projectPathConfig);
		...
		ProjectPathConfig projectPathConfig) throws OperationException
	{
		...
		if (recordParam != null)
		{
			...
			if (record != null)
			{
				...
				{
					String projectType = record.getString(projectPathConfig.getProjectProjectTypeFieldPath());
	...
	// This does something similar to ProjectUtil.setCurrentProjectType but it only needs to set it in the master,
	// and it does it for multiple records at once in same procedure, so it's different enough that it's worth having separate code.
	private static void setCurrentProjectType(
		...
		final String value) throws OperationException
	{
		if (subjectRecords.isEmpty())
		{
			return;
		...
		Adaptation masterDataSet = masterDataSpace.findAdaptationOrNull(firstSubjectRecord.getContainer()
			.getAdaptationName());
		AdaptationTable masterTable = masterDataSet.getTable(firstSubjectRecord.getContainerTable()
			.getTablePath());
		final List<Adaptation> masterSubjectRecords = new ArrayList<Adaptation>();
		for (Adaptation subjectRecord : subjectRecords)
		{
			Adaptation masterSubjectRecord = masterTable.lookupAdaptationByPrimaryKey(subjectRecord.getOccurrencePrimaryKey());
		...
		Procedure proc = new Procedure()
		{
			...
			public void execute(ProcedureContext pContext) throws Exception
			{
				...
				{
					final Path currentProjectTypePath = subjectPathConfig.getSubjectCurrentProjectTypeFieldPath();
					String currentProjectType = subjectRecord.getString(currentProjectTypePath);
					if (!ObjectUtils.equals(currentProjectType, value))
					{
						HashMap<Path, Object> pathValueMap = new HashMap<Path, Object>();
						pathValueMap.put(currentProjectTypePath, value);
