...
/**
 */
public abstract class NewProjectWorkflowLauncherForm
//extends WorkflowLauncherForm
{
	...
	/*
			...
			writer.add_cr(
				"<input id='" + SubjectWorkflowLauncher.PARAM_PROJECT_NAME + "' name='"
					...
					+ (initialProjectName == null ? ""
						: " value=\"" + StringEscapeUtils.escapeHtml(initialProjectName) + "\"")
					+ "/>");
			writer.endFormRow();
		}
		...
		/*	protected void writeLauncherHiddenInputs(ServiceContext sContext)
			{
				writeHiddenInput(sContext, WorkflowLauncher.PARAM_XPATH_TO_TABLE, getXPathToTable());
