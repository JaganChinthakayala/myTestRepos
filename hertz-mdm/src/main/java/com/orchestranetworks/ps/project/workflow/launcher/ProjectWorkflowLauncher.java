...
public abstract class ProjectWorkflowLauncher extends WorkflowLauncher
	implements ProjectPathCapable
{
	...
	public void execute(WorkflowLauncherContext context) throws OperationException
	{
		...
		super.execute(context, getProjectMasterWorkflowKey(), pathConfig.getProjectTablePath()
			.format(), null);
	}
	...
	{
		String projectType = workflowLauncherContext.getCurrentAdaptation().getString(
			getProjectPathConfig().getProjectProjectTypeFieldPath());
	...
	protected void setAdditionalContextVariables() throws OperationException
	{
		...
		launcher.setInputParameter(
			...
			workflowLauncherContext.getCurrentAdaptation().getString(
				getProjectPathConfig().getProjectProjectTypeFieldPath()));
		launcher.setInputParameter(
			ProjectWorkflowConstants.PARAM_RESUMING,
			String.valueOf(resuming));
	}

	@Override
	protected void assignWorkflowDescription()
	{
		this.workflowDescription = workflowLauncherContext.getCurrentAdaptation().getString(
			getProjectPathConfig().getProjectProjectTypeFieldPath());
	}
	...
	protected void preventMultipleLaunches() throws OperationException
	{
		...
		String projectType = adaptation.getString(projectPathConfig.getProjectProjectTypeFieldPath());
		...
		if (subjectPath == null)
		{
			...
			{
				subjectRecords = new ArrayList<Adaptation>();
			}
			else
			{
		...
		if (subjectRecords.size() > 0)
		{
			...
			if (currentProjectTypePath != null)
			{
				...
				ProcedureExecutor.executeProcedure(
					proc,
					workflowLauncherContext.getSession(),
					workflowLauncherContext.getCurrentDataSpace());
			}
		}
	}
	...
	private class SetCurrentProjectTypeProcedure implements Procedure
	{
		...
		public void execute(ProcedureContext pContext) throws Exception
		{
			...
			pathValueMap.put(currentProjectTypePath, projectType);
			for (Adaptation subjectRecord : subjectRecords)
			{
				...
				{
					throw OperationException.createError("A workflow is already in process for the selected project.");
				}
				ModifyValuesProcedure.execute(pContext, subjectRecord, pathValueMap);
			}
		}
	}
