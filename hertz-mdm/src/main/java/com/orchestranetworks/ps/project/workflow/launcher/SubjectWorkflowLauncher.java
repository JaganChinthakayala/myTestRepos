	...
	implements ProjectPathCapable, SubjectPathCapable
{
	...
	protected void setAdditionalContextVariables() throws OperationException
	{
		...
		{
			launcher.setInputParameter(
				ProjectWorkflowConstants.PARAM_RECORD_NAME_VALUE,
				projectName);
		}
	}
	...
	protected void preventMultipleLaunches() throws OperationException
	{
		...
		if (subjectPathConfig != null)
		{
			...
			if (newSubjectProjectType == null
				|| !workflowLauncherContext.getParameter(PARAM_PROJECT_TYPE).equals(
					newSubjectProjectType))
			{
	...
	{
		String currentProjectType = subjectRecord.getString(getSubjectPathConfig().getSubjectCurrentProjectTypeFieldPath());
		return currentProjectType == null ? null : UserMessage.createError("A "
			+ currentProjectType + " workflow is already in process for the selected record.");
	}
