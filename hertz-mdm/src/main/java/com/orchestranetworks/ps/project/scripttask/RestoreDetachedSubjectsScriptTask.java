	...
	implements ProjectPathCapable
{
	...
	{
		Path pathToSubject = getPathToSubject(subjectRelatedRecord.getContainerTable()
			.getTablePath());
		...
		throws OperationException
	{
		...
		Path projectProjectSubjectsPath = pathConfig.getProjectProjectSubjectsFieldPath(projectType);
		// TODO: Should we also handle a direct fk to subject (such as in Restaurant)? Don't have need for it now since
		//       we don't let them pick a different value after launching.
		if (projectProjectSubjectsPath == null)
		{
			throw OperationException.createError("This script task can only be used with a project that has detachable subjects.");
		}
		List<Adaptation> currentSubjects = AdaptationUtil.getLinkedRecordList(
			projectRecord,
			projectProjectSubjectsPath);
		HashSet<PrimaryKey> currentSubjectPKs = new HashSet<PrimaryKey>();
		for (Adaptation currentSubject : currentSubjects)
		{
			currentSubjectPKs.add(currentSubject.getOccurrencePrimaryKey());
		}

		HashSet<PrimaryKey> processedSubjectPKs = new HashSet<PrimaryKey>();

		DifferenceBetweenInstances dataSetDiff = DifferenceHelper.compareInstances(
			dataSet,
			initialDataSet,
			false);
		...
		Set<PrimaryKey> currentSubjectPKs) throws OperationException
	{
		...
		if (!processedSubjectPKs.contains(subjectRecordPK))
		{
			...
			{
				AdaptationTable subjectTable = projectRecord.getContainer().getTable(
					subjectTablePath);
				Adaptation subjectRecord = subjectTable.lookupAdaptationByPrimaryKey(subjectRecordPK);
	...
	//		SubjectPathConfig subjectPathConfig = projectPathConfig.getSubjectPathConfig(getSubjectTablePath(projectRecord));
	...
	//					+ " contains an unknown primary key column. It should be a link table containing just a project column and subject column.");
	//			}
	//		}
	//		return projectSubjectTable.lookupAdaptationByPrimaryKey(projectSubjectTable.computePrimaryKey(pkValues));
	//	}

	protected Adaptation deepCopy(ProcedureContext pContext, Adaptation initialSubjectRecord)
		throws OperationException
	{
		DeepCopyService service = new DeepCopyService(getDeepCopyConfigFactory());
