...
public class ProjectWorkflowPermissionsInfo
{
	...
	// Use sets for roles & users so if a user id or role name is added twice, it will only be in there once.
	// Would like to be able to determine permissions per work item, but that's not possible with the API.
	// If a user has permission for any of the work items, he has permission for all of them.
	protected Set<Role> roles = new HashSet<Role>();
	protected Set<String> userIds = new HashSet<String>();

	protected Set<ProcessInstanceKey> subWorkflowKeys = new HashSet<ProcessInstanceKey>();
	...
	public void addWorkItemsUsersAndRoles(Set<UserReference> users)
	{
		...
		// If role should be added when all work items are allocated or there's no new user (which means it's
		// offered or about to be offered)
		if (!workItems.isEmpty()
			&& (users == null || users.isEmpty() || shouldAddRoleWhenAllWorkItemsAllocated(workItems)))
		{
	...
	public void addDataContextUsers(DataContextReadOnly dataContext)
	{
		...
		String currentUserId = dataContext.isVariableDefined(WorkflowConstants.PARAM_CURRENT_USER_ID) ? dataContext.getVariableString(WorkflowConstants.PARAM_CURRENT_USER_ID)
			: null;
		...
		{
			String projectTeamMembers = dataContext.getVariableString(ProjectWorkflowConstants.PARAM_PROJECT_TEAM_MEMBERS);
			if (projectTeamMembers != null)
			{
				String[] arr = projectTeamMembers.split(String.valueOf(ProjectWorkflowConstants.PROJECT_TEAM_MEMBERS_PARAM_SEPARATOR));
				for (String str : arr)
				{
					userIds.add(str);
				}
	...
	{
		// Don't need to recurse for these work item related permissions because it would always be based on
		...
		return !(WorkflowPermission.ALLOCATE.equals(permissionType)
			...
			|| WorkflowPermission.REALLOCATE.equals(permissionType) || WorkflowPermission.MANAGE_ALLOCATION.equals(permissionType));
	}
