...
public class ProjectWorkflowActionPermissions extends DefaultWorkflowActionPermissions
{
		...
		WorkflowPermission permissionType)
	{
		...
		// Create the permissions info and fill it with data context users BEFORE getting the process instance,
		// because that is expensive on performance
		ProjectWorkflowPermissionsInfo permissionsInfo = notCachingPermissionPreProcessInstanceFetch(context);
	...
	// This essentially does the same as isUserPermittedWhenNotCaching, except it can be called from the recursive check
	// through the subworkflows because at that point we have the process instance. We DON'T want to simply call this from
	// the isUserPermittedWhenNotCaching even though that would make for prettier code because we want to separate the
	// pre-process fetch & post-process fetch logic in that case. We can't just use isUserPermittedWhenNotCaching because we don't
	...
	private boolean isUserPermittedForSubworkflowWhenNotCaching(
		...
		WorkflowPermission permissionType)
	{
		ProjectWorkflowPermissionsInfo permissionsInfo = notCachingPermissionPreProcessInstanceFetch(dataContext);
		if (permissionsInfo.isUserPermitted(session, permissionType))
		{
			return true;
		}
	...
	// Do the logic that can be done prior to fetching the process instance, since that is expensive on performance.
	...
	private ProjectWorkflowPermissionsInfo notCachingPermissionPreProcessInstanceFetch(
		DataContextReadOnly dataContext)
	{
		ProjectWorkflowPermissionsInfo permissionsInfo = getPermissionsInfoFactory().createPermissionsInfo(
			null);
		permissionsInfo.addDataContextUsers(dataContext);
		return permissionsInfo;
	}

	// Do the logic that can be done after fetching the process instance. This entails adding work item users & roles and
		...
		WorkflowPermission permissionType)
	{
		...
		if (!completed)
		{
			...
			permissionsInfo.addWorkItemsUsersAndRoles(WorkflowUtilities.getWorkItemUsers(workItems));
			if (permissionsInfo.isUserPermitted(session, permissionType))
			{
				return true;
			}
			// If we should recurse through subworkflows then check each subworkflow. This recursively will end up calling
