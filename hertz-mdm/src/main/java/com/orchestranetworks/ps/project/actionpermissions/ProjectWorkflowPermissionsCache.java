...
// TODO: Refactor the whole project workflow cache to not require a project. For the most part, it's relevant to any workflow
//       with a record, not specifically a project record. Subclass will handle the project team member stuff.
//       Will require a path config specific to workflow events. I believe this can all be done without affecting the interface
//       currently used by workflows, thus being backwards-compatible.
/**
 * Stores users allowed to view/allocate workflows in memory for faster reads during permissions checks.
...
public class ProjectWorkflowPermissionsCache
{
	...
	// Key = the key of the workflow instance, Value = a ProjectWorkflowPermissionsInfo object containing the users & roles
	private final HashMap<ProcessInstanceKey, ProjectWorkflowPermissionsInfo> cacheMap = new HashMap<ProcessInstanceKey, ProjectWorkflowPermissionsInfo>();
	...
	public static ProjectWorkflowPermissionsCache getInstance()
	{
		...
		if (instance == null)
		{
			synchronized (ProjectWorkflowPermissionsCache.class)
			{
				...
				instanceRef = new SoftReference<ProjectWorkflowPermissionsCache>(instance);
			}
		}
		return instance;
	}
		...
		WorkflowPermission permissionType)
	{
		...
		// If we aren't supposed to recurse subworkflows for this permission, then they're not permitted
	...
	public final ProjectWorkflowPermissionsInfo refreshCache(
		...
		ProjectWorkflowPermissionsInfoFactory permissionsInfoFactory)
	{
		ProjectWorkflowPermissionsInfo permissionsInfo = permissionsInfoFactory.createPermissionsInfo(processInstance);
