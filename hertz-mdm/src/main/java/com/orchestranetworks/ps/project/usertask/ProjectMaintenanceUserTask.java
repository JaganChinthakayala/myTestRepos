	...
	implements ProjectPathCapable
{
		...
		UserReference user) throws OperationException
	{
		...
		// The user is valid if there is no project team member user or there is but he is not that user
	...
	{
		return ProjectUtil.getProjectTeamMemberRecord(
			projectRecord,
			userRole,
			getProjectPathConfig());
	}
	...
	public void checkBeforeWorkItemCompletion(UserTaskBeforeWorkItemCompletionContext context)
	{
		...
		if (cancelable
			...
			&& ((!acceptIsCancel && !context.isAcceptAction()) || (acceptIsCancel && context.isAcceptAction())))
		{
		...
		Session session)
	{
		...
		try
		{
			projectRecord = ProjectWorkflowUtilities.getProjectRecord(
				context,
				repository,
				pathConfig);
		}
		...
		{
			String cancelReason = projectRecord.getString(pathConfig.getProjectCancelReasonFieldPath());
			if (cancelReason == null)
			{
				String wfComment = session.getInteraction(true).getComment();
				if (wfComment == null)
				{
					return UserMessage.createError("A cancel reason must be entered when cancelling a project.");
				}
			}
		}
		return null;
	...
	public void handleWorkItemCompletion(final UserTaskWorkItemCompletionContext context)
		throws OperationException
	{
		final Adaptation projectRecord = ProjectWorkflowUtilities.getProjectRecord(
			context,
			context.getRepository(),
			getProjectPathConfig());
		if (cancelable)
		...
		if (projectRecord != null)
		{
			final Role role = WorkflowUtilities.getWorkItemOfferedToRole(context.getCompletedWorkItem());
		...
		Repository repo) throws OperationException
	{
		...
		if ((!acceptIsCancel && workItem.isRejected()) || (acceptIsCancel && workItem.isAccepted()))
		{
			...
			if (shouldSendToCancellationProcess(projectRecord))
			{
				if (cancelReasonRequired)
				{
					...
					if (cancelReason == null)
					{
						...
						if (comment != null)
						{
							SchemaNode cancelReasonNode = projectRecord.getSchemaNode().getNode(
								cancelReasonPath);
							...
							pathValueMap.put(cancelReasonPath, comment);
							ModifyValuesProcedure.execute(projectRecord, pathValueMap, session);
						}
					}
				}
			}
			...
			else
			{
				...
				// For non-new subject projects, clear the current project type from the subject records.
				// This should work whether the context contains a project record or a subject record.
		...
		// Only set isCancelled in the data context if one or more work items were rejected or logically cancelled
		...
		{
			HashMap<Path, Object> pathValueMap = new HashMap<Path, Object>();
			pathValueMap.put(projectPathConfig.getProjectCancelReasonFieldPath(), null);
			ModifyValuesProcedure.execute(projectRecord, pathValueMap, session);
		}
	}
		...
		boolean overwrite) throws OperationException
	{
		...
		if (teamMember != null)
		{
			...
			if (teamMemberUserId == null || overwrite)
			{
				...
				pathValueMap.put(userIdPath, user.getUserId());
				ModifyValuesProcedure.execute(pContext, teamMember, pathValueMap);
			}
		}
	}
		...
		ProjectPathConfig pathConfig)
	{
		...
		List<Adaptation> teamMembers = AdaptationUtil.getLinkedRecordList(
			projectRecord,
	...
	private void addAllProjectTeamUsers(UserTaskCreationContext context) throws OperationException
	{
		...
		for (Adaptation teamMember : teamMembers)
		{
			String roleName = teamMember.getString(pathConfig.getProjectTeamMemberProjectRoleFieldPath());
