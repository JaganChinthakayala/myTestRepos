...
public class SetLocationIntelligentLocationIdsScriptTask extends ScriptTask
{
	@Override
	public void executeScript(ScriptTaskContext context) throws OperationException
	{
		//Read all of the Location Records marked for Resetting
		...
		RequestResult requestResult = locationTable.createRequestResult(
			LocationPaths._Root_Location._ResetIntelligentIds.format() + "=true");
		...
		for (int i = 0; i < requestResult.getSize(); i++)
		{
			...
			HtzLocationUtilities
				.alignIntelligentLocationIdsWithLocationTypes(locationRecord, null, context);
