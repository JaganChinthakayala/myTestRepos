...
public class LocationPossibleAreasTableRefFilter implements TableRefFilter
{
	@Override
	public boolean accept(Adaptation record, ValueContext context)
	{
		String locationPath = null;
		boolean isAreaInAreaBandingRange = true;
		...
		isAreaInAreaBandingRange = HtzLocationUtilities
			.isAreaInAreaBandingRange(areaRecord, locationRecord);

		if (!isAreaInAreaBandingRange)
		{
			return false;
		}
		...
		for (int i = 0; i < areaHierarchyPaths.size(); i++)
		{
			...
			isHierarchyValidForThisAreaType = HtzLocationUtilities.isHierarchyValidForThisAreaType(
				locationPath,
				LocationConstants.OPERATION_AREA_TYPE_AREA);
