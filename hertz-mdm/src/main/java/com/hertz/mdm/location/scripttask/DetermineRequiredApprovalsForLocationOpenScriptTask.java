...
public class DetermineRequiredApprovalsForLocationOpenScriptTask extends ScriptTask
{
	@Override
	public void executeScript(ScriptTaskContext context) throws OperationException
	{
		...
		{
			approvalRoles = approvalRoles + delimiter
				+ LocationConstants.APPROVAL_ROLE_FACILITIES_CONSTRUCTION;
		}

		if (projectRecord
			.get_boolean(LocationPaths._Root_LocationProject._RequiresEnvironmentalAffairsApproval))
		{
			approvalRoles = approvalRoles + delimiter
				+ LocationConstants.APPROVAL_ROLE_ENVIRONMENTAL_AFFAIRS;
		}
