 ...
 */
package com.hertz.mdm._hertz.usertask;
...
public abstract class HtzProjectMaintenanceUserTask extends ProjectMaintenanceUserTask
{
	...
	/**
	 * Return the path to the brand field, used by the primary user lookup table.
	 * If {@ link #isPrimaryUserTableUsed()} returns <code>false</code>, then this is not checked and can return <code>null</code>.
	 * Default is <code>null</code>.
	 *
	 * @return the path to the brand field, or <code>null</code> if no primary user table is used
	 */
	protected Path getBrandFieldPath()
	{
		return null;
	}
	...
	{
		Role lookupRole = userRole;

		UserReference projectTeamUser = super.getProjectTeamUserForRole(
			projectRecord,
			context,
			lookupRole);

		if (!primaryUserTableUsed || projectTeamUser != null)
		{
